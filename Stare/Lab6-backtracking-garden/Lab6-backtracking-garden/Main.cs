...
namespace lab7
{
    ...
    class MainClass
    {
        public static void Main(string[] args)
        {
            ...
            Console.Out.WriteLine("Duże testy");
        ...
        static bool PerformTest(int num, Test test)
        {
            ...
            foreach (var solution in solutions)
            {
                ...
                if (solution.Length != test.solutionSize)
                    Console.Out.WriteLine("Test {0}: Rozwiązanie o rozmiarze {1}, oczekiwane {2}", num, solution.Length, test.solutionSize);
            }

            if (sizeCorrect && solutionsCorrect && solutions.Count != test.solutionsCount)
            {
                Console.Out.WriteLine("Test {0}: znalezione rozwiązenie poprawne, Z£A LIBA ROZWIĄZAÑ {1}, oczekiwane {2}", num, solutions.Count, test.solutionsCount);
                return false;
            }
            if (sizeCorrect && solutionsCorrect)
            {
                Console.Out.WriteLine("Test {0}: znalezione rozwiązenie poprawne, liczba OK -- {1}", num, solutions.Count);
                return true;
            }
            if (!sizeCorrect)
                Console.Out.WriteLine("Test {0}: nieoptymalne rozwiązanie. Oczekiwany rozmiar: {1}", num, test.solutionSize);
            if (!solutionsCorrect)
                Console.Out.WriteLine("Test {0}: błędne rozwiązanie.", num);
            return false;
        }
    }
}
